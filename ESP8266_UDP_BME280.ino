...
void loop()
{
  ...
  if ( noBytes ) {
    ...
    //--------------BME280-Temperature: -40…85°C 2Byte DPT9.001---------------//
    if  ((tlength == 1) && (apci == 0) && (gamain == 4) && (gamiddle == 0) && (gasub == 0)) {  // if "1bit" + "Read" + "4/0/0"
      Serial.print("Response - 4/0/0 - BME280 - Temperature: ");
      UDPipaction2 = 1;
      ...
      UDPvalueT = (bme.readTemperature() + calibrationT);
      F01a = 0; // [1bit]
      F01b = 0; // [4bit]
      F01c = 0; // [11bit]
    ...
    //--------------BME280-hum-0...100% 1Byte DPT5.001--------------//
    if  ((tlength == 1) && (apci == 0) && (gamain == 5) && (gamiddle == 0) && (gasub == 0)) {  // if "1bit" + "Read" + "5/0/0"
      Serial.print("Response - 5/0/0 - BME280 - Relative humidity: ");
      UDPipaction2 = 1;
    ...
    //--------------BME280-Pressure: 300...1100 hPa 2Byte DPT9.006---------------//
    if  ((tlength == 1) && (apci == 0) && (gamain == 5) && (gamiddle == 1) && (gasub == 0)) {  // if "1bit" + "Read" + "5/1/0"
      Serial.print("Response - 5/1/0 - BME280 - Pressure: ");
      UDPipaction2 = 1;
      UDPdpt = 3; // 1=1bit 2=1Byte 3=2Byte
      UDPipaction2 <<= 4;
      packetBufferWrite[5] = UDPipaction2 | UDPdpt;    // 11 UDPnet/IP action [+4bit], Total length [4bit]
      ...
      packetBufferWrite[14] = UDPat | UDPnpci | UDPdpt;    //1[HEX] AT [1bit], NPCI [3bit], Length(DPT) [4bit]
      packetBufferWrite[16] = 64;  // APCI(Type) [+2 bit], DATA/APCI [6 bit]
      UDPvalueP = (bme.readPressure() / 100.0F);
      F02a = 0; // [1bit]
      F02b = 0; // [4bit]
      F02c = 0; // [11bit]
  ...
  if (D1miniTime > nextTime) {
    ...
    //--------------BME280-Temperature: -40…85°C 2Byte DPT9.001---------------//
    Serial.print("Write - 4/0/0 - BME280 - Temperature: ");
    UDPipaction2 = 1;
    UDPdpt = 3; // 1=1bit 2=1Byte 3=2Byte
    UDPipaction2 <<= 4;
    packetBufferWrite[5] = UDPipaction2 | UDPdpt;    // 11 UDPnet/IP action [+4bit], Total length [4bit]
    ...
    delay(5);
    //--------------BME280-hum-0...100% 1Byte DPT5.001--------------//
    Serial.print("Write - 5/0/0 - BME280 - Relative humidity: ");
    UDPipaction2 = 1;
    UDPdpt = 2; // 1=1bit 2=1byte
    UDPipaction2 <<= 4;
    packetBufferWrite[5] = UDPipaction2 | UDPdpt;    // 11 UDPnet/IP action [+4bit], Total length [4bit]
    ...
    //--------------BME280-Pressure: 300...1100 hPa 2Byte DPT9.006---------------//
    Serial.print("Write - 5/1/0 - BME280 - Pressure: ");
    UDPipaction2 = 1;
    UDPdpt = 3; // 1=1bit 2=1Byte 3=2Byte
    UDPipaction2 <<= 4;
    packetBufferWrite[5] = UDPipaction2 | UDPdpt;    // 11 UDPnet/IP action [+4bit], Total length [4bit]
